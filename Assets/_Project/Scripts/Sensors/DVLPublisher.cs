...
public class DVLPublisher : ROSPublisher
{
    ...
    [Tooltip("Minimum update rate (Hz) - used when simulateAdaptiveRate is false")]
    [Range(1f, 50f)]
    public float fixedUpdateRate = 10f;
    ...
    protected override void Start()
    {
        ...
        if (SimulationSettings.Instance.PublishDynamicDVLRate)
        {
            // DVL uses its own adaptive timing based on altitude if option is on, not base class rate limiting
            useBaseRateLimiting = false;
        }
        else
        {
            // If not simulating adaptive rate, we can still use base class timing for customizability
            useBaseRateLimiting = true;
            PublishRate = SimulationSettings.Instance.DvlRate;
        }
    ...
    protected override void FixedUpdate()
    {
        ...
        if (Time.time >= nextPublishTime && SimulationSettings.Instance.PublishDynamicDVLRate)
        {
        ...
        else
        {
            ...
            if (SimulationSettings.Instance.PublishDVL && SimulationSettings.Instance.PublishROS)
            {
                base.FixedUpdate(); // Let base class handle fixed update timing when not using adaptive rate
            }
        }
