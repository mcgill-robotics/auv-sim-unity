...
public class DVLPublisher : ROSPublisher
{
    ...
    protected override void FixedUpdate()
    {
        ...
        currentSensorPos = delayBuffer != null ? delayBuffer.GetDelayedSensorPosition(localSensorOffset, 0.15f) : transform.position;
        ...
        if (IsValid)
        {
            Vector3 currentVel = delayBuffer != null ? delayBuffer.GetThrottledDelayedVelocityAtLocalOffset(localSensorOffset, 0.15f) : AuvRb.GetPointVelocity(transform.position);
    ...
    private void ProcessVelocitySample()
    {
        ...
        else
        {
            // No samples (maybe newly enabled?), just process "instant" current
             Vector3 currentVel = transform.InverseTransformDirection(delayBuffer != null ? delayBuffer.GetThrottledDelayedVelocityAtLocalOffset(localSensorOffset, 0.15f) : AuvRb.GetPointVelocity(transform.position));
