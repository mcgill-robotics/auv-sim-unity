...
public class SensorDelayBuffer : MonoBehaviour
{
    ...
    [Header("Domain Randomization Settings")]
    [Tooltip("If true, randomizes delayTime and dvlPingInterval upon reset")]
    public bool randomizeLatencyOnReset = false;
    [Tooltip("Minimum latency delay in seconds when randomized")]
    [Range(0.05f, 0.3f)]
    public float minDelayTime = 0.10f;
    [Tooltip("Maximum latency delay in seconds when randomized")]
    [Range(0.05f, 0.3f)]
    public float maxDelayTime = 0.20f;
    [Tooltip("Minimum DVL acoustic ping interval when randomized")]
    [Range(0.1f, 0.3f)]
    public float minDvlPingInterval = 0.15f;
    [Tooltip("Maximum DVL acoustic ping interval when randomized")]
    [Range(0.1f, 0.3f)]
    public float maxDvlPingInterval = 0.22f;
    ...
    /// Also applies domain randomization to latency and acoustic ping rate if enabled.
    /// </summary>
    public void ResetState()
    {
        if (randomizeLatencyOnReset)
        {
            delayTime = UnityEngine.Random.Range(minDelayTime, maxDelayTime);
            dvlPingInterval = UnityEngine.Random.Range(minDvlPingInterval, maxDvlPingInterval);
            Debug.Log($"[SensorDelayBuffer] Randomized latency to {delayTime * 1000f:F1}ms and DVL ping interval to {dvlPingInterval * 1000f:F1}ms.");
        }
    ...
    /// If delaySeconds < 0, uses the configured (or randomized) delayTime.
    /// </summary>
    public RigidbodyStateSample GetDelayedState(float delaySeconds = -1f)
    {
        if (delaySeconds < 0f) delaySeconds = delayTime;
    ...
    /// </summary>
    public Vector3 GetDelayedSensorPosition(Vector3 localSensorOffset, float delaySeconds = -1f)
    {
    ...
    /// </summary>
    public Vector3 GetThrottledDelayedVelocityAtLocalOffset(Vector3 localSensorOffset, float delaySeconds = -1f)
    {
        RigidbodyStateSample sample = GetDelayedState(delaySeconds);
    ...
    /// </summary>
    public Vector3 GetUnthrottledDelayedVelocityAtLocalOffset(Vector3 localSensorOffset, float delaySeconds = -1f)
    {
        RigidbodyStateSample sample = GetDelayedState(delaySeconds);
        Vector3 r = sample.rotation * localSensorOffset;
